...
function wp_unregister_GLOBALS() {
	...
	if ( isset($_REQUEST['GLOBALS']) )
		die('upptäckte försök att skriva över GLOBALS');
...
if ( version_compare( '4.2', phpversion(), '>' ) ) {
	die( 'Din server har PHP version ' . phpversion() . ' men WordPress behöver åtminstone 4.2.' );
}

if ( !extension_loaded('mysql') && !file_exists(ABSPATH . 'wp-content/db.php') )
	die( 'Din PHP-installation verkar sakna MySQL som WordPress behöver.' );
...
if ( preg_match('|[^a-z0-9_]|i', $wpdb->prefix) && !file_exists(ABSPATH . 'wp-content/db.php') )
	wp_die("<strong>FEL</strong>: <code>$table_prefix</code> i <code>wp-config.php</code> kan endast innehålla siffor, bokstäver och _-tecken.");
